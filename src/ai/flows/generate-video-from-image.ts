...
const generateVideoFromImageFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const imageDataUri = image.url;

    // Save the generated image to Firestore, but only if it's initialized
    // Note: We are saving an image to the 'videos' collection for simplicity.
    // In a real app, this might be a separate 'images' collection.
    const firestoreReady = await isFirestoreAvailable();
    if (firestoreReady) {
        try {
            await createVideo({
              userId: input.userId,
              videoUri: imageDataUri, // Saving image URI in videoUri field
